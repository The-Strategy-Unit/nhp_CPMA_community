# Quantifying potentially mitigable hospital activity: community specific categories
...
The report can be found at: <https://connect.strategyunitwm.nhs.uk/nhp/CPMA_community>.
...
`create-mitigator-reports.R` is a script to create the 29 categories of potentially mitigable activity and the 4 mechanism quarto files. Each of these quarto files uses child documents to create the various sections of the report. These child documents can be found in the `child-dir` folder and are numbered to match the function files for convenience.
