...
avg_los_england <- targets::tar_read(avg_los_england) |>
  dplyr::filter(mitigator == cohort) 

avg_los_icb <- targets::tar_read(avg_los_icb) |>
  dplyr::filter(mitigator == cohort) 
```
...
perc_los |>
  ...
  flextable::add_footer_lines(glue::glue("Average length of stay in 2023/24: {avg_los_england_2324} days"))
```
