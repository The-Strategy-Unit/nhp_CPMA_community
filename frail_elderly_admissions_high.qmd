...
icb_population_data<-tar_read(icb_population_data)
  ...
  data_number_percentage_over_time_icb(denominator_over_time, 
                                       numbers_over_time, 
                                       icb_population_data, 
                                       "frail_elderly_high", 
                                       episodes, 
                                       total_episodes_emergency)|>
        mutate(activity=number)|>
plotting_icb_over_time(data, 'Number')
...
#| fig-cap: The percentage of total emergency admissions that are for Frail Elderly High potentially mitigable activity by ICB over the last 9 years. 
 ...
 data_number_percentage_over_time_icb(denominator_over_time, 
                                       ...
                                       episodes, 
                                       total_episodes_emergency)|>
        mutate(activity=percentage)|>
plotting_icb_over_time(data, 'Percentage')

```

```{r}

test<- data_number_percentage_over_time_icb(denominator_over_time, 
                                       numbers_over_time, 
                                       icb_population_data, 
                                       "frail_elderly_high", 
                                       episodes, 
...
plotting_icb_over_time(
  (age_sex_standardised_rates_episodes|> filter(cohorts=="frail_elderly_high")),
  value)

test2<-age_sex_standardised_rates_episodes|>
...
age_sex_standardised_rates_episodes|>
  ...
  group_by(icb_2024_name)|>
    ...
    plot_ly( x = ~year, y = ~value, type = 'scatter',  mode = 'lines', text=~icb_2024_name,  line = list(color = "#686f73",  width = 1.5))|>
    ...
    layout(
      ...
      yaxis = list(title = 'Standardised rate per 100,000', showline = TRUE, showgrid = F , linewidth=2 ,zeroline = FALSE, tickformat = "digits", anchor="free", shift=100)
    )
