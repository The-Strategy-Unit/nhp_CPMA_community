...
Of the 2023/24 mitigable admissions for frail and elderly (high) cohort, most (`r perc_frail_elderly_high_los_admissions |> dplyr::filter(weeks == 1) |> dplyr::pull(perc_weeks) |> purrr::pluck(1)`%) length of stays are under 1 week and `r perc_frail_elderly_high_los_admissions |> dplyr::filter(los_range == 0) |> dplyr::pull(perc)`% are less than 1 day.  
...
targets::tar_read(perc_frail_elderly_high_los_admissions) |>
...
```{r}
perc_frail_elderly_high_los_beddays <- targets::tar_read(perc_frail_elderly_high_los_beddays)
```

```{r}
#| label: tbl-perc_frail_elderly_high_los_beddays
#| tbl-cap: !expr get_caption_by_group("perc", "Frail and Elderly (high)", "beddays", "los", "emergency")
targets::tar_read(perc_frail_elderly_high_los_beddays) |>
  get_perc_by_los_table("beddays")
```

```{r}
#| label: fig-perc_frail_elderly_high_los_beddays
#| fig-cap: !expr get_caption_by_group("perc", "Frail and Elderly (high)", "beddays", "los", "emergency")
perc_frail_elderly_high_los_beddays |>
  get_perc_by_los_plot("beddays")
```
...
age_sex_standardised_rates_episodes<-tar_read(age_sex_standardised_rates_episodes)
...
plotting_percentage_change_over_time_by_icb(age_sex_standardised_rates_episodes, value, england_average_standardised_episodes$change, 1.3, 1.7)
...
age_sex_standardised_rates_beddays<-tar_read(age_sex_standardised_rates_beddays)
...
plotting_percentage_change_over_time_by_icb(age_sex_standardised_rates_beddays, value, england_average_standardised_beddays$change, 1.4, 1.6)
